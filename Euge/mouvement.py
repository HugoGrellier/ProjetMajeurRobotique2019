...
def move_middle_obj(motionService):
    ...
    move(1.01,0,motionService,True)
...
def move_right_obj(motionService):
    ...
    move(1.01,0,motionService,False)
    Grab_obj(motionService)
    move(-1.01,0.40,motionService,True)

def move_left_obj(motionService):
    motionService.setAngles(["LShoulderPitch", "LShoulderRoll","LHand"], [0.02,0,4], [1,1,1])
    move(0,-0.15,motionService,True)
    move(1.02,0,motionService,False)
    Grab_obj(motionService)
    move(-1.02,0,motionService,True)

def move_to_Blue(motionService):
